...
export default function SimpleWordGame() {
  ...
  const gameContainerRef = useRef<HTMLDivElement>(null); // Renamed from gameRef for clarity
    ...
    if (element) {
      ...
      if (cellCoords) {
        ...
        if (selectedLetters.length > 0) {
          const lastSelected = selectedLetters[selectedLetters.length - 1];
          // Call handleMouseOver only if it's a new cell
  ...
  const getCellClass = (row: number, col: number) => {
    ...
    if (isCellSelected(row, col)) {
      return `${baseClasses} ${responsiveSizeClasses} bg-gradient-to-br from-primary to-purple-500 dark:from-primary dark:to-indigo-700 text-white shadow-md transform scale-105 sm:scale-110`;
    ...
    <div ref={gameContainerRef} className="w-full flex flex-col items-center">
      ...
      <div className="flex flex-col md:flex-row gap-4 md:gap-8 justify-center items-start w-full px-2">
        <div 
          ...
          style={{ touchAction: 'none' }} // Prevents browser default touch actions like pinch-zoom on the grid area.
        ...
        >
          <div className="overflow-x-auto"> {/* Allows horizontal scrolling for the grid if it's too wide */}
            <div 
              ...
              {grid.map((row, rowIndex) => (
                row.map((letter, colIndex) => (
                  <div
                    ...
                    onMouseOver={() => handleMouseOver(rowIndex, colIndex)} // Mouse drag over cells
                    onTouchStart={() => handleMouseDown(rowIndex, colIndex)} // Touch start initiates selection
                    // onTouchMove is now handled by the parent grid container
                  >
                    {letter}
                  </div>
                ))
